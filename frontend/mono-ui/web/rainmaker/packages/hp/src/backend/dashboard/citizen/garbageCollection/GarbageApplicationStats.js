...
import GarbageApplicationList from "./GarbageApplicationList";


const GarbageApplicationStats = () => {
	...
	useEffect(() => {
		(async () => {
			const citizenDash = await getCitizenGarbageAppStats();
	...
	const getStatuswiseApplications = async (status) => {
		const citizenDash = await getCitizenGarbageAppStats(`?accountId=${JSON.parse(localStorage.getItem("userInfo")).uuid}&status=${status}`);
		if (citizenDash) {
			setApplicationLists(citizenDash.garbageAccounts);
		}
	};
...
export default GarbageApplicationStats;
